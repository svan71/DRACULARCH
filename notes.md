...
Last updated: 2025-12-08 (session 8 - Switched from Fish to Bash)

## CURRENT SESSION STATUS
**Dracula.sh switched from Fish to Bash with ble.sh - ready for fresh install test**

### Completed this session:
1. ✅ Discovered ble.sh makes bash feel like fish (autosuggestions, syntax highlighting)
2. ✅ Added thefuck and tldr packages
3. ✅ Synced all fish aliases/functions to bash
4. ✅ Replaced setup_fish() with setup_bash() in Dracula.sh
5. ✅ Removed fish package, added bash-completion, thefuck, tldr
6. ✅ Updated Carapace config for bash
7. ✅ Updated Ghostty shell-integration to "detect"
8. ✅ Renamed repo folders: dracula/ → Dracula/, mokka/ → Mokka/
9. ✅ Created bash_history in repo for fresh install suggestions

### Why Bash over Fish:
- ble.sh provides fish-like experience (autosuggestions, syntax highlighting)
- POSIX compatible - scripts from anywhere just work
- One shell to maintain, not two
- Same look/feel with Starship prompt

### Next steps:
1. Test Dracula.sh on fresh Arch install
2. If successful: implement same Bash changes in Mokka.sh
3. Sync to Synology backup
